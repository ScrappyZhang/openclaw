...
/**
 * Zod issue type for unrecognized_keys errors from strict() schemas.
 * Contains the actual property names that caused the validation failure.
 */
...
function isUnrecognizedKeysIssue(record: UnknownIssueRecord | null): record is UnrecognizedKeysIssue {
  return record?.code === "unrecognized_keys" && Array.isArray(record.keys);
}

function formatUnrecognizedKeys(keys: PropertyKey[]): string {
  return keys
...
function mapZodIssueToConfigIssue(issue: unknown): ConfigValidationIssue {
  ...
  // Enhance unrecognized_keys errors to show the actual property names
  if (isUnrecognizedKeysIssue(record)) {
    const formattedKeys = formatUnrecognizedKeys(record.keys);
    message = `unrecognized properties: ${formattedKeys}`;
  }
